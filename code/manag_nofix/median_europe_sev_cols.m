%Calculate median in countries for files with several value columns
%Marieke Scheel

function [fin_av]=median_europe_sev_cols(cmass_years, cntrcol)
...
for i=1:nbrRows
    ...
    elseif sort_cmass_years(i,cntrcol)~=cntr %when country changes get median
        ...
        cmass_europe(count,cntrcol-2)=sort_cmass_years(i-1,cntrcol); %country FAO number
        addedmort=nan(500,cntrcol-4);
        row_count=1;
        count=count+1;
